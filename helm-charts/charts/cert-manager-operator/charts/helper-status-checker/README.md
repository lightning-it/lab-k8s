...
helper-status-checker will create a Service Account (including a namespaced Role and RoleBinding) and a Job that will try to check the status of the Operator. If the Operator is not available after some time (configurable with mx_retries), the Job will fail.
...
| checks[0].installCSV | string | `"installCSV"` | OPTIONAL: CSV to be installed |
...
checks:
  - operatorName: name-of-operator
    ...
    installCSV: name-of-operator.v0.11.0
